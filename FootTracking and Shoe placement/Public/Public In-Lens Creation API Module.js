 ...
 * // Instead, you should modify the "Public In-Lens Creation API" script and access the functions through the imported class wrapper.
*/

/*
 * @param {RemoteApiResponse} response A raw API response from an Public In-Lens Creation API
...
ApiModule.prototype.observe_edits = function(cb) {
    var req = global.RemoteApiRequest.create();
    req.endpoint = "observe_edits";
...
ApiModule.prototype.open_keyboard = function(cb) {
    var req = global.RemoteApiRequest.create();
    req.endpoint = "open_keyboard";
    this.remoteServiceModule.performApiRequest(req, function(response) {
        if(cb) {
            handleAPIResponse(response, cb);
        }
