...
class BackgroundRenderer {
    ...
    /**
     * Draws a Camera2 texture with the given transform matrix (for VIO mode).
     */
    fun drawCamera2Texture(textureId: Int, transformMatrix: FloatArray) {
        // Transform texture coordinates using the SurfaceTexture transform matrix
        texCoordsBuffer.position(0)
        transformedTexCoordsBuffer.position(0)
        
        // Apply transform matrix to texture coordinates
        val tempCoords = FloatArray(8)  // 4 vertices * 2 coords
        texCoordsBuffer.get(tempCoords)
        
        // Transform each texture coordinate
        for (i in 0 until 4) {
            val x = tempCoords[i * 2]
            val y = tempCoords[i * 2 + 1]
            // Apply 2D transform (matrix is 4x4, we use top-left 2x2 for 2D transform)
            val tx = transformMatrix[0] * x + transformMatrix[4] * y + transformMatrix[12]
            val ty = transformMatrix[1] * x + transformMatrix[5] * y + transformMatrix[13]
            tempCoords[i * 2] = tx
            tempCoords[i * 2 + 1] = ty
        }
        
        transformedTexCoordsBuffer.put(tempCoords)
        transformedTexCoordsBuffer.position(0)
